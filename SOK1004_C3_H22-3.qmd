...
Ligning 4 viser veksbidraget prosentvis, med en 12 måndeders-endring.
...
df1 <- df %>% 
  pivot_wider(names_from = konsumgruppe, values_from = val)
df1[is.na(df1)] <- 0
...
#aviikene er mellom 0.06 og 0.02
```
...
df2 <- select(df2,-var,-dato)

df2 <- df2/1000

df4 <- select(df4,-var,-dato)

df5 <- df2 * df4

df5 <- df5 %>% 
  mutate(Total = rowSums(df5)) %>%
  relocate("Total", .before="Brød og kornprodukter")

df6 <- select(df3,var,dato)
...
Ifølge oppdateringen på canvas skal totalindeksen i 2015 tilsvare 80, vi kan da se at utregningen er korrekt.
...
df8 <- df7 %>%
  select("dato", "Total", `Elektrisitet inkludert nettleie`) %>%
   mutate(Ltotalindeks = lag(Total,12)) %>%
   mutate(Leotalindeks = lag(`Elektrisitet inkludert nettleie`,12)) %>%
   mutate(prosentvisendring = 100*(Total-Ltotalindeks)/Ltotalindeks)  %>%
   mutate(eprosentvisendring = 100*(`Elektrisitet inkludert nettleie`-Leotalindeks)/Leotalindeks)  %>%
   pivot_longer(6:7, names_to = "prosent", values_to = "prosentvisendring") %>%
...
Figuren viser at konsumgruppen "Elektrisitet, inkludert nettleie." samsvarer kpi nært, og endringene i prosent har en vis korrelasjon.
...
Fra august 2020 til august 2021 steg kpi med 3.4 prosentpoeng. I denne perioden steg prisene på elektrisitet inkludert nettleie med 97.9 prosent, der kraftprisene hadde størst innvirkning på dette. (ssb, https://www.ssb.no/priser-og-prisindekser/konsumpriser/statistikk/konsumprisindeksen/artikler/hoye-strompriser-preger-kpi-i-august). Elektrisitet har høy vekt i forhold til andre konsumgrupper , dermed vil kraftprisene ha mye å si for kpi i prosentvisendring.
